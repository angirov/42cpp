...
private:
    ...
    void _handle_input_line(std::string &line, char delim)
    {
        ...
        std::getline(ss, segment, ',');
        if (!__isNumeric(__trim(segment))) {
            std::cerr << "ERROR: invalid number format: " << segment << std::endl;
            return;
        }
        float amount = handle_float(segment);
        if (amount < 0) {
            std::cerr << "ERROR: not a positive number.: " << segment << std::endl;
            return;
        }
        if (amount > 1000) {
            std::cerr << "ERROR: too large a number: " << segment << std::endl;
            return;
        }
        std::cout.width(4);
