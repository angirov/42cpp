...
public:
    void _load_csv(char *filepath)
    {
        ...
        if (debug) std::cout << " ====== Finished initing DB ======" << std::endl;
    }
    ...
    void _load_line(std::string &line, char delim)
    {
        ...
        if (debug) std::cout << "DEBUG: loading data: date: " << segment;
        ...
        if (debug) std::cout << " set to: " << price << std::endl;
    ...
    void process_input(char *filepath)
    {
        std::string line;
        std::ifstream infile(filepath);
    ...
    void handle_input_line(std::string &line, char delim)
    {
        ...
        std::getline(ss, segment, delim);
        if (!__valid_date(segment))
            std::cerr << "ERROR: processing input: invalid date: " << segment;
        std::string date = segment;
        std::cout << "date: " << segment;
...
int main(int argc, char **argv)
{
    ...
    Btc btc;
    btc._load_csv(csv_path);
    btc.process_input(input_path);
    // btc.iter_db();
    ...
    btc.__valid_date("2022-12-02");
    btc.__valid_date("2222-12-02");
    btc.__valid_date("2022-13-02");
    btc.__valid_date("2022-13-42");
